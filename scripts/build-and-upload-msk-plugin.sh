...
#   - build/configs/table-create.moonlink.json
...
# Avro table schema (required for loadgen + Moonlink table creation)
...
if [[ -z "$AVRO_SCHEMA_JSON_COMPACT" || "$AVRO_SCHEMA_JSON_COMPACT" == "null" ]]; then
  echo "table.avro_schema missing in config; required for record.schema.json and table creation payload" >&2
  exit 1
fi

# Optional table storage S3 config (used to build table_config in create payload)
...
jq -n \
  ...
  --arg tasks "$SRC_TASKS" \
  --arg mps "$SRC_MPS" \
  --arg size "$SRC_MSG_SIZE" \
  --arg dur "$SRC_MAX_SECS" \
  --arg run "$SRC_RUN_INDEFINITE" \
  --arg topic "$SRC_TOPIC" \
  --arg sr "$SCHEMA_REGISTRY_URL" \
  --arg record_schema "$AVRO_SCHEMA_JSON_COMPACT" \
  '{name: $name, config: {
  ...
  }}' > "$CONFIG_DIR/source-connector.msk.json"
...
# 3b) Generate Moonlink table-create JSON payload (Avro schema-based)
TABLE_CREATE_JSON="$CONFIG_DIR/table-create.moonlink.json"
...
- Table create:  $CONFIG_DIR/table-create.moonlink.json
EOM
